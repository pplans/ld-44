	...
	SubShader
	{
			...
			half frag(v2f i) : COLOR
			{
				...
				for (int k = 0; k<NumberOfIterations; k += 1)
				{
					...
					ColorIntensityInRadius += tex2D(
						_MainTex,
						i.uv.xy + float2((k - NumberOfIterations / 2)*TX_x, 0)
		...
		#include "UnityCG.cginc"
			...
			half4 frag(v2f i) : COLOR
			{
				...
				half4 outcolor = ColorIntensityInRadius * half4(0,0,1,1) * 2 + (1 - ColorIntensityInRadius)*tex2D(_SceneTex, i.uv.xy);
				return outcolor;
			}
			ENDCG
		}
